/*1:
Write a function compareToTen that takes a number, num as an argument and returns a Promise 
that tests if the value of num is less than or greater than the value 10.
if number is greater or equal it should resolve  with a string and log for example, 
if num = 20 it should say '20 is greater than or equal to 10, resolved!'
...
let compareToTen = new Promise((resolve, reject) => {
  if (num >= 10) {
...
let drink = new Promise((resolve, reject) => {
  console.log(`I'll be right back with you ${order}.`);
...
let arr = complicatedArray;

let words = new Promise((resolve, reject) => {
  results = [];
  arr.map(item => {
    typeof item === "string" ? results.push(true)
                            : results.push(false)
    } 
);
  results.includes(false) ? reject("Error: Not All Items are strings.")
...
/*4.
  ...
  b. Within the promise create a reject handler for errors and a resolve handler that resolves a 
  copy of the array that is inside the json object.
  ...
  f. Chain again and find all the names of the people in the legal industry and log those names in a 
  string: "Contacts from the legal profession:" + list all of the names
  g. In that same thenable return 30% of each of the sales and pass the value to the next thenable
  h. Add all of the sales totals together and return a string saying: "Total sales for everyone from 
  the Legal profession after taxes is $30000" if the value of the added sales is 30000 */
...
totalSales.then((dataArr) => {
  ...
  console.log('Legal Team Contacts:');
  allLegal.forEach(person => console.log(`${person.contact}`));
  return allLegal.map(sale => (sale.sales) * .3); //?
}).then((bigSales) => {
  let answer = bigSales.reduce((total, current) => total + current, 0); //?
  console.log(`Total sales for everyone from the Legal profession after taxes is $${answer}.`)
})
.catch((errMessage) => {
...
/* 5. Based on given athlets array
  a.  Write a function called playerFunction that returns a promise which copies the array into a 
      new array called 'playerArr'.
      Make sure your function is re-useable so no hard coded values.
  b. Now invoke the function
      loop through the function using array method and Place a key value pair into each object based 
      ...
      Note: There can only be two sports either football or basketball and only two positions guard 
      and quarterback
  c.
      Log (console.log) the whole array with the sport added for each object. Preface the list with 
      a 'Sport added: ' string.
  ...
  d.
      ...
      Only choose the players who play football and use deconstruction and the ternary operator to 
      complete this piece
